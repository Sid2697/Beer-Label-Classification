...
parser.add_argument('-l', default='lookup/main_orb.pkl', help='Path to the pre computed descriptors')
...
for query_name in tqdm(os.listdir(query_images)):
    ...
    _, q_desc = orb.detectAndCompute(query, None)
    for database_name in database_descriptors.keys():
        ...
        matches = flann.knnMatch(np.asarray(q_desc,np.float32), np.asarray(d_desc,np.float32), k=2)
